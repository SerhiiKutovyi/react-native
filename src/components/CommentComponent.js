...
export default function CommentComponent({ item }) {
  ...
  return (
    <View
      style={{
        ...styles.container,
        flexDirection: userId !== myId ? 'row' : 'row-reverse',
      }}
    >
      <Image
        source={{ uri: userPhoto ? userPhoto : 'https://i.pravatar.cc/300' }}
        style={{
          ...styles.avatar,
          marginLeft: userId !== myId ? 0 : 16,
          marginRight: userId !== myId ? 16 : 0,
        }}
      />
      <View
        style={{
